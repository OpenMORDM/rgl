...
convertBBox <- function(id) {
  ...
  if (length(mat$color) > 1)
    mat$color <- mat$colors[2] # We ignore the "box" colour
                               ...
                               
{
  ...
  init <- function(id, type, flags) {
    ...
    if (type == "surface") { # Compute both indices
  ...
  scriptEnd <- subst(
    ...
    canvas.addEventListener("touchmove", function(e) { e.button=1; canvas.onmousemove(e); }, false);
